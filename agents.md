...
### 10. 任务牌（Quest）与卡牌转换（勤学 DiligentStudy → 博识 Erudition）

> 对照原版 探寻 Dowsing → 富足 Abundance。参考实现：`TheSolitaryCode/Cards/DiligentStudy.cs`、`TheSolitaryCode/Cards/Erudition.cs`。

- **任务牌骨架**：`CardType.Quest` + `CardRarity.Quest` + 费用 `-1` + `CanonicalKeywords => [CardKeyword.Unplayable]` + `MaxUpgradeLevel => 0` + `CanBeGeneratedInCombat => false`，注册进**原版 `QuestCardPool`**（`[RegisterCard(typeof(QuestCardPool))]`，与探寻同池）→ 不进奖励 / 商店 / 随机变化。
