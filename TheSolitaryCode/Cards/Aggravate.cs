...
// 造成 8 点伤害；目标身上每有一种负面效果，所有负面效果数值 +1（升级后 +2）——
// 即目标身上的负面效果种类越多，本次增幅越大（可叠加放大）。
// 参考熔岩之拳 MoltenFist（负面效果层数操控）与腐蚀之拳 CorrosiveFist（枚举 Debuff 型 Power 逐个 +1）。
...
public sealed class Aggravate : ModCardTemplate
{
	...
	// 负面效果数值增量的 DynamicVar 键名（绑定 {DebuffAmount:diff()} 占位符）。
	...
	// 基础数值：伤害 8 + 负面效果数值增量 1（绑定 {Damage:diff()} / {DebuffAmount:diff()} 占位符）。
	...
	// 打出时：先造成 8 点伤害，再按目标身上的负面效果种类数放大负面效果增幅：
	// 目标每有一种负面效果，所有负面效果数值 +DebuffAmount（升级后每种子 +2）。
	protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
	{
		...
		// 每种负面效果数值 + DebuffAmount × 负面效果种类数。
		// 用 ModelDb.DebugPower + ToMutable 生成可变原型传给 PowerCmd.Apply：
		// 已存在的负面效果会走叠加路径加层数；理论上可能存在的 Instanced 型负面效果也会安全地新增一层。
