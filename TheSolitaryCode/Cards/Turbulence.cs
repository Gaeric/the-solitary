...
// 湍流（character.org 金卡，英文名 Turbulence）：2 费技能。
// 获得 8 点格挡（升级后 10），抽 2 张牌；每当你洗牌 1 次，这张牌耗能减少 1（本场战斗内可叠加，战斗结束自动失效）。
...
public sealed class Turbulence : ModCardTemplate
{
	...
	// 基础数值：格挡 8（升级后 10）+ 抽牌数 2（绑定 {Block:diff()} / {Cards:diff()} 占位符）。
