...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/TheSolitary/wiki/Home) | [English Home](https://github.com/alkaid616/TheSolitary/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `TheSolitary`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `TheSolitary.json` and update `id`, `name`, `author`, and `description`.
3. Edit `TheSolitaryCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `TheSolitary/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `TheSolitary.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `TheSolitary.json` has been synced to the resolved `STS2.RitsuLib` version.
...
| Command | Behavior |
|---|---|
| `dotnet build .\TheSolitary.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/TheSolitary` directory.
...
```text
TheSolitary/
├── TheSolitaryCode/   # C# source
├── TheSolitary/       # Godot resources, localization, and placeholder scenes
├── TheSolitary.csproj
├── TheSolitary.json   # Mod manifest
├── project.godot
└── local.props.template
```

`res://TheSolitary/...` is the Godot/PCK resource path, mapping to the repository resource directory `TheSolitary/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
**Scenes** (`res://TheSolitary/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `TheSolitary_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `TheSolitary_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `TheSolitary_merchant.tscn` | Merchant | — |
| `TheSolitary_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `TheSolitary_character_select_bg.tscn` | Character select background | — |
...
`TheSolitary.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:

```json
{
  "id": "TheSolitary",
  "name": "TheSolitary",
  "pck_name": "TheSolitary",
  "author": "Author",
